...
#let isource(uid, position, ..params) = {
    ...
    let draw(variant, scale, rotate, ..styling) = {
        ...
        if (variant == "iec") {
            ...
            line((0, -radius + padding), (rel: (0, 2*radius - 2*padding)))
            mark((rel: (0, 0)), (rel: (0,1)), symbol: ">", fill: black, scale: arrow-scale)
        }
    }
...
#let vsource(uid, position, ..params) = {
    ...
    let draw(variant, scale, rotate, ..styling) = {
        ...
        if (variant == "iec") {
            ...
            line((rel: (-sign-size , -radius + padding)), (rel:  (2*sign-size, 0)))
            line((-sign-size, radius - padding - sign-delta),(rel: (2*sign-size, 0)))
            line((rel: (-sign-size, sign-size)),(rel: (0, -2*sign-size)))
        }
    }
