...
#import cetz.draw: anchor, circle, floating, line, mark, scale as cetzscale, translate
    ...
    let draw = (
        anchors: (ctx, position, variant, scale, rotate, wires, ..styling) => {
            ...
            if (envelope) {
                ...
                anchor("emitter", (
                    emitter.at(0),
                    emitter.at(1) + wires-direction * wires-length,
                ))
                anchor("collector", (
                    collector.at(0),
                    collector.at(1) - wires-direction * wires-length,
                ))
